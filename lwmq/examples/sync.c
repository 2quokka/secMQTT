...
int main() {
  ...
  for (;;) {
    ...
    if (available > 0) {
      err = lwmqtt_yield(&client, available, COMMAND_TIMEOUT, NULL);
      if (err != LWMQTT_SUCCESS) {
    ...
    if (lwmqtt_unix_timer_get(&timer3) <= 0) {
      ...
      //lwmqtt_message_t msg = {.qos = LWMQTT_QOS0, .retained = false, .payload = (uint8_t *)("world"), .payload_len = 5};
		...
		//topic init
		//topic secure option check -s
		bool s_opt = false;
		lwmqtt_topic_check(TOPIC, &s_opt);

		lwmqtt_message_t *msg = lwmqtt_create_msg(LWMQTT_QOS0, false, (uint8_t *)("this is test"), s_opt, U_NAME); 
		printf("create msg\n");
		printf("payload : %s \n", msg->payload);
		printf("len : %d \n", (int)msg->payload_len);
		printf("hash : %s \n", msg->digest);
